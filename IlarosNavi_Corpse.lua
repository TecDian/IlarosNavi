...
local function SetCorpseArrow()
	if c and z and x and y and c > 0 and z > 0 and x > 0 and y > 0 then
		uid = IlarosNavi:AddZWaypoint(c, z, x*100, y*100, L["My Corpse"], false, true, true, nil, true, true)
		return uid
	end
end
...
eventFrame:SetScript("OnUpdate", function(self, elapsed)
	...
	if counter < throttle then
		...
		counter = 0
		if IlarosNavi.profile.general.corpse_arrow then
...
eventFrame:SetScript("OnEvent", function(self, event, arg1, ...)
	if event == "ADDON_LOADED" and arg1 == "IlarosNavi" then
...
if IsLoggedIn() then
	eventFrame:GetScript("OnEvent")(eventFrame, "ADDON_LOADED", "IlarosNavi")
end
