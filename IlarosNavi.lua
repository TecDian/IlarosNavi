	...
	[1] = {
		...
		{ -- Remove waypoint
			...
			func = function()
				...
				--IlarosNavi:PrintF("Removing waypoint %0.2f, %0.2f in %s", data.x, data.y, data.zone) 
			end,
		},
...
function IlarosNavi:CHAT_MSG_ADDON(event, prefix, data, channel, sender)
	...
	local msg = string.format(L["|cffffff78IlarosNavi|r: Waypoint '%s' in zone %s was sent by %s"], title, zone, sender)
...
function IlarosNavi:AddZWaypoint(c, z, x, y, desc, persistent, minimap, world, custom_callbacks, silent, crazy)
	...
	if not silent and self.profile.general.announce then
        ...
        local msg = string.format(L["|cffffff78IlarosNavi:|r Added a waypoint '%s%s%s in %s"], desc and desc or "", desc and L["' at "] or "", ctxt, zone)
		ChatFrame1:AddMessage(msg)
	end
...
local function usage()
	ChatFrame1:AddMessage(L["|cffffff78IlarosNavi |r/Navi |cffffff78Usage:|r"])
	ChatFrame1:AddMessage(L["|cffffff78/Navi <x> <y> [desc]|r - Adds a waypoint at x,y with descrtiption desc"])
	ChatFrame1:AddMessage(L["|cffffff78/Navi <zone> <x> <y> [desc]|r - Adds a waypoint at x,y in zone with description desc"])
	ChatFrame1:AddMessage(L["|cffffff78/Navi reset all|r - Resets all waypoints"])
	ChatFrame1:AddMessage(L["|cffffff78/Navi reset <zone>|r - Resets all waypoints in zone"])
end
