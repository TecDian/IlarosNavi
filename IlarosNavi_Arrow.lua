...
local wayframe = CreateFrame("Button", "IlarosNaviCrazyArrow", UIParent)
...
local function OnDragStart(self, button)
	if not IlarosNavi.db.profile.arrow.lock then
		self:StartMoving()
	end
end
...
local function OnEvent(self, event, ...)
	if event == "ZONE_CHANGED_NEW_AREA" and IlarosNavi.profile.arrow.enable then
		self:Show()
	end
end
...
function IlarosNavi:SetCrazyArrow(uid, dist, title)
	active_point = uid
...
local function OnUpdate(self, elapsed)
	...
	local dist,x,y = IlarosNavi:GetDistanceToWaypoint(active_point)
	...
	if not dist or IsInInstance() then
		if not IlarosNavi:IsValidWaypoint(active_point) then
			active_point = nil
			-- Change the crazy arrow to point at the closest waypoint
			if IlarosNavi.profile.arrow.setclosest then
	...
	if dist <= arrive_distance then
		...
		local angle = IlarosNavi:GetDirectionToWaypoint(active_point)
		...
		local gr,gg,gb = unpack(IlarosNavi.db.profile.arrow.goodcolor)
		local mr,mg,mb = unpack(IlarosNavi.db.profile.arrow.middlecolor)
		local br,bg,bb = unpack(IlarosNavi.db.profile.arrow.badcolor)
...
function IlarosNavi:ShowHideCrazyArrow()
	if self.profile.arrow.enable then
		wayframe:Show()
		...
		wayframe:SetScale(IlarosNavi.db.profile.arrow.scale)
		wayframe:SetAlpha(IlarosNavi.db.profile.arrow.alpha)
		local width = IlarosNavi.db.profile.arrow.title_width
		local height = IlarosNavi.db.profile.arrow.title_height
		local scale = IlarosNavi.db.profile.arrow.title_scale

		wayframe.title:SetWidth(width)
		wayframe.title:SetHeight(height)
		titleframe:SetScale(scale)
		titleframe:SetAlpha(IlarosNavi.db.profile.arrow.title_alpha)
...
local dropdown_info = {
	...
	[1] = {
		{
			-- Title
			text = L["IlarosNavi Waypoint Arrow"],
			isTitle = 1,
		},
		{
			...
			func = function()
				...
				active_point = nil
				if IlarosNavi.profile.arrow.setclosest then
					local uid = IlarosNavi:GetClosestWaypoint()
...
local function WayFrame_OnClick(self, button)
	if active_point then
		if IlarosNavi.db.profile.arrow.menu then
			UIDropDownMenu_Initialize(IlarosNavi.dropdown, init_dropdown)
...
local function wayframe_OnEvent(self, event, arg1, ...)
	if arg1 == "IlarosNavi" then
		if IlarosNavi.db.profile.feeds.arrow then
			...
			local feed_crazy = ldb:NewDataObject("IlarosNavi_CrazyArrow", {
				...
				OnTooltipShow = function(tooltip)
					local dist = IlarosNavi:GetDistanceToWaypoint(active_point)
					if dist then
			...
			local throttle = IlarosNavi.db.profile.feeds.arrow_throttle
			local counter = 0

			function IlarosNavi:UpdateArrowFeedThrottle()
				throttle = IlarosNavi.db.profile.feeds.arrow_throttle
			end
...
--  Example Usage:
--
--  if not IlarosNavi:CrazyArrowIsHijacked() then
--    IlarosNavi:HijackCrazyArrow(function(self, elapsed)
--      -- Random angle
--      local angle = math.random(math.pi * 2)
--      IlarosNavi:SetCrazyArrowDirection(angle)
--      -- Random color
--      local r = math.random(100) / 100
--      local g = math.random(100) / 100
--      local b = math.random(100) / 100
--      IlarosNavi:SetCrazyArrowColor(r, g, b)
--      -- Titles
--      IlarosNavi:SetCrazyArrowTitle("Hijacked arrow", "Hijacked", "You will never arrive")
--    end)
--  end
--
--  -- At some point later
--  IlarosNavi:ReleaseCrazyArrow()
-------------------------------------------------------------------------]]--
...
function IlarosNavi:SetCrazyArrowDirection(angle)
    cell = floor(angle / twopi * 108 + 0.5) % 108
    local column = cell % 9
    local row = floor(cell / 9)
...
function IlarosNavi:SetCrazyArrowColor(r, g, b, a)
    arrow:SetVertexColor(r, g, b, a)
end

-- Convenience function to set the title/status and time to arrival text
-- of the crazy arrow.
function IlarosNavi:SetCrazyArrowTitle(title, status, tta)
...
function IlarosNavi:HijackCrazyArrow(onupdate)
    wayframe:SetScript("OnUpdate", onupdate)
